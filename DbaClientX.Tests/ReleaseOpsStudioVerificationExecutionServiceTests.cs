...
public sealed class ReleaseOpsStudioVerificationExecutionServiceTests
{
    [Fact]
    ...
    private static TemporaryModuleScope CreateTemporaryModule(string moduleName, string version, string preRelease)
    {
        ...
        File.WriteAllText(
            ...
            "    }" + Environment.NewLine +
            "}" + Environment.NewLine);
        File.WriteAllText(Path.Combine(moduleRoot, $"{moduleName}.psm1"), "function Test-ReleaseOpsStudio { }");
