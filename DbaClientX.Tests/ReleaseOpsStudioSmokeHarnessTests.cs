...
public sealed class ReleaseOpsStudioSmokeHarnessTests
{
    [Fact]
    public async Task DbaClientX_LocalSmokePath_ExercisesBuildAndRetryStages()
    {
        EnsureSmokeEnabled();
        ...
        Assert.Contains(buildResult.AdapterResults, result => result.AdapterKind == ReleaseBuildAdapterKind.ProjectBuild && result.Succeeded);
        Assert.NotEmpty(buildResult.AdapterResults);
        Assert.Contains(buildResult.AdapterResults, result => result.ArtifactFiles.Any(file => file.EndsWith(".nupkg", StringComparison.OrdinalIgnoreCase)));
    ...
    private static string ResolveRepositoryRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(current.FullName, "DbaClientX.sln"))
        ...
        throw new DirectoryNotFoundException("Unable to locate the DbaClientX repository root for the smoke harness.");
    }
