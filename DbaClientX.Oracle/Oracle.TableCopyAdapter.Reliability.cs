  ...
  AND ROWNUM = 1";
        ...
        IReadOnlyList<DbaColumnInfo> destinationColumns)
    {
        ...
        foreach (DbaColumnInfo identity in destinationColumns.Where(column =>
                     column.IsIdentity == true &&
                     string.Equals(column.IdentityGeneration, "BY DEFAULT ON NULL", StringComparison.OrdinalIgnoreCase)))
        {
            if (!projectedColumns.TryGetValue(identity.Name, out DataColumn? sourceColumn)) continue;
            if (!page.Rows.Cast<DataRow>().Any(row => row.IsNull(sourceColumn))) continue;

            throw new InvalidOperationException(
                $"Oracle destination '{tableName}' projects NULL for identity column '{identity.Name}', which is GENERATED BY DEFAULT ON NULL. " +
                "ClearDestination cannot safely preflight this value because the backing sequence is not rolled back. " +
                "Project a non-null explicit value or copy without ClearDestination.");
        }
    }
