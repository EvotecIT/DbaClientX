        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (DbaTableCopyDefinition definition in definitions)
        {
            ...
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                int? precision = reader.IsDBNull(1) ? null : Convert.ToInt32(reader.GetValue(1));
                int? scale = reader.IsDBNull(2) ? null : Convert.ToInt32(reader.GetValue(2));
                if (IsPortableOracleNumber(precision, scale) || IsPortableNumericProjection(definition, column)) continue;
