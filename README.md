...
Supported PostgreSQL arrays, geometric, range/multirange, text-search, bit-string, and other provider-native values remain lossless when the destination is PostgreSQL. Copies to other providers reject those declarations before writing unless the column is excluded or explicitly converted to `String`.
